  ...
  import com.eatza.order.model.Order; import com.eatza.order.model.OrderedItem;
  import com.eatza.order.repository.OrderedItemRepository; import
  com.eatza.order.service.itemservice.ItemServiceImpl;
  
  @RunWith(MockitoJUnitRunner.class)
  
  @SpringBootTest public class ItemServiceTest {
  
  @Mock OrderedItemRepository itemRepository;
  ...
  { 
	  ...
	  OrderedItem orderedItem = new OrderedItem( "name",  1,  12,  order, 1L);
  orderedItem.setName("Dosa"); orderedItem.setId(1L);
  orderedItem.setOrder(order); orderedItem.setPrice(100);
  ...
  assertEquals("Dosa", returnedItem.getName()); assertEquals(order,
  returnedItem.getOrder()); assertEquals(2, returnedItem.getQuantity()); }
  
  
  @Test public void findByOrderId_basic() { Order order = new Order(1L,
  "UPDATED", 1L); OrderedItem orderedItem = new OrderedItem("Dosa", 2, 100,
  order, 1L); List<OrderedItem> orderedItems = new ArrayList<>();
  ...
  @Test public void deleteItemsById() {
